	...
	(function($,window){
		...
		window.InputTask={
            ...
            bindEvent:function(){
                ...
                self.$input.focus(function(){ //高亮后就显示弹出框
		            ...
		            if (self.$input.val().trim()) {   //判断，空字符不弹出框
		                self.timeoutid = setTimeout(self.ajax_request, 100);
		            }
                });
                self.$input.blur(function(){
                    setTimeout(self.clear, 200);  //失去焦点后，干掉弹出框
                });
                ...
                self.$input.keyup(function (event) {
		            if (event.keyCode > 40 || event.keyCode == 8 || event.keyCode == 32) {  //字母数字，退格，空格
		                ...
		                if (self.$input.val().trim()) {   //判断，空字符不弹出框
		                    self.timeoutid = setTimeout(self.ajax_request, 100);  //控制请求频率
		                }
		            }
            ...
            setSelectedItem:function(item){
	            ...
	            self.$popmenu.find('li').removeClass('highlight').eq(selectedItem).addClass('highlight');
            },
